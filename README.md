...
| `enable-policy-deletion` | no | `'false'` | Make a securable's configured mask/filter policies authoritative: any actual policy on that securable not declared in config is deleted. A securable with an explicit empty `policies: []` has all its policies removed; one that omits `policies` is left untouched. Requires interactive confirmation unless `force: 'true'` — in CI you must set `force` or the run errors out |
...
Policy fields:
- **`name`** — the policy name (required on all policy types). Must be unique among the policies attached to the same securable (catalog/schema/table).
...
- **`privileges`** — (`grant` type only) the UC privileges to assign. Supported values include the concrete UC privileges (`select`, `modify`, `create_table`, `create_schema`, `create_function`, `create_volume`, `use_catalog`, `use_schema`, `read_volume`, `write_volume`, `execute`, `refresh`, `create_materialized_view`, `create_model`, `create_model_version`, `create_feature`, `read_feature`, `browse`, `all_privileges`, `external_use_schema`, `manage`) and four shorthand **abstractions** that each expand to a fixed set of UC privileges:
...
Mask and filter policies are additive by default (create/update, never delete). With `--enable-policy-deletion`, a securable's configured policies become **authoritative**: any actual mask/filter policy on that securable not declared in config is deleted. A securable that omits the `policies` field (`None`) is left unmanaged, whereas one declaring an explicit list — including an empty `policies: []`, or a list holding only grant policies — has its mask/filter policies reconciled (an empty desired set removes them all). Deletions are read from UC via the policies SDK (`list_policies`), scoped by `--delete-policies-for-namespaces`, and — like governed-tag deletion — require an interactive `y`/`yes` confirmation at the terminal unless `--force` is set (in non-interactive contexts you must set `--force` or the run errors out). Grant policies are unaffected here; they remain managed (with revokes) by the privileges domain.
...
- **Privileged-action opt-in flags** — five classes of mutation are gated behind explicit CLI flags. Each defaults to `false`; if unset, the corresponding actions are **skipped in both dry runs and real runs** — no fetch, no diff, no log section, no SQL. Pass the flag to opt in:
  ...
  - `--enable-policy-deletion` — make a securable's configured mask/filter policies authoritative and `DROP` any actual policy on that securable not declared in config. **A securable declaring `policies: []` (or a list containing only grant policies) has *all* its mask/filter policies removed; a securable that omits `policies` entirely is left unmanaged.** Authority keys on whether the `policies` field is present (not `None`). Actual policies are read via the policies SDK (`list_policies`), including for securables that declare an empty list. Scoped by `--delete-policies-for-namespaces`. The engine logs the policies slated for deletion and requires an interactive `y`/`yes` confirmation before issuing any `DROP POLICY`; pair with `--force` in non-interactive contexts. Grant policies are unaffected — they stay managed (with revokes) by the privileges domain.
  ...
  **Scoping filters for the four `--enable-...` flags above:**
  ...
  - `--delete-policies-for-namespaces <cat_a,cat_b.sch>` — scope of `--enable-policy-deletion`. Only securables in-scope have their configured policies treated as authoritative; out-of-scope securables never have policies deleted (their `policies` list, if any, still drives create/replace).
